//! Swap Pricer - discount-rate sensitivities of a simple interest rate swap.
//!
...
//!
//!   2. **Gamma** (second-order, diagonal) via the dedicated `Jet2` forward
//!      type — one forward pass per rate produces both the first and second
//!      derivative w.r.t. that rate exactly (no finite differences). This
//!      recovers only the diagonal `d²v/dr_i²`, at `n_rates` passes.
//!
//!   3. **The full `n × n` Hessian** via `compute_hessian_k::<8, _>` — the
//!      K-wide forward-over-adjoint engine, whose tape storage scalar carries
//!      8 tangent lanes at once. It recovers *every* cross-gamma
//!      `d²v/dr_i dr_j` in `ceil(30/8) = 4` record-and-sweep passes, i.e.
//!      strictly more output than mode 2 for a fraction of the passes. Its
//!      diagonal is cross-checked against mode 2 below.
//!
//!
...
use xad_rs::Jet2;
use xad_rs::math;
use xad_rs::{compute_hessian_k, JetK, Tape, TapeStorage};
...
/// Prices a simple interest-rate swap using `Jet2<f64>` for second-order
/// forward-mode differentiation.
///
/// Uses the same discount-factor sharing trick as the reverse-mode pricer.
///
/// The returned `Jet2` carries:
///   - `value()`              = swap value `v`
///   - `first_derivative()`   = `dv/dr_i` for the `Jet2::variable`-seeded rate
...
fn price_swap_jet2(
    ...
    // Shared discount factors: discounts[t] = (1 + r_t)^mat_t
    let discounts: Vec<Jet2<f64>> = (0..n)
        .map(|t| {
    ...
    let mut b_fix = Jet2::constant(0.0);
    let mut b_flt = Jet2::constant(0.0);
    for t in 0..n {
        b_fix += Jet2::constant(fixed_cashflow) / discounts[t];
        b_flt += Jet2::constant(face_value * float_rates[t]) / discounts[t];
    }

    let last_discount = discounts[n - 1];
    b_fix += Jet2::constant(face_value) / last_discount;
    b_flt += Jet2::constant(face_value) / last_discount;
...
fn main() {
    ...
    // ========================================================================
    // Pass 2 — Gamma (diagonal) via Jet2 second-order forward mode
    // ========================================================================
    //
    // For each rate r_i, we seed `disc_rates_jet2[i] = Jet2::variable(r_i)`
    // and leave all other rates as `Jet2::constant`. One forward pass then
    ...
    for _ in 0..N_TRIALS {
        for i in 0..n_rates {
            let disc_rates_jet2: Vec<Jet2<f64>> = disc_rates
                .iter()
                .enumerate()
                .map(|(k, &r)| {
                    if k == i {
            ...
            if i == 0 {
                value_fwd = v_jet2.value();
            }
            deltas_fwd[i] = v_jet2.first_derivative();
            gammas[i] = v_jet2.second_derivative();
        }
        black_box(&gammas);
    }
    ...
    // ========================================================================
    // Pass 3 — Full n x n Hessian via the K-wide forward-over-adjoint engine
    // ========================================================================
    //
    // `compute_hessian_k::<8, _>` instantiates the reverse tape over a storage
    // scalar carrying 8 tangent lanes (`JetK<f64, 8>`), so one recording plus
    // one sweep yields 8 Hessian columns. The whole 30 x 30 matrix therefore
    // costs ceil(30/8) = 4 passes — every cross-gamma, not just the diagonal.
    //
    // The pricer body is the SAME generic `price_swap<T: TapeStorage>` the
    // reverse-mode pass used; only the storage scalar differs. The passive
    // inputs are lifted into the lane-carrying scalar as constants.
    const K: usize = 8;
    let mats_k: Vec<JetK<f64, K>> = maturities.iter().map(|&m| JetK::constant(m)).collect();
    let floats_k: Vec<JetK<f64, K>> = float_rates.iter().map(|&r| JetK::constant(r)).collect();
    let fixed_k = JetK::<f64, K>::constant(fixed_rate);
    let face_k = JetK::<f64, K>::constant(face_value);

    let n_blocks = n_rates.div_ceil(K);
    let hess_trials = N_TRIALS / 100;
    let t_hess = Instant::now();
    let mut hessian = compute_hessian_k::<K, _>(&disc_rates, |v| {
        price_swap(v, is_fixed_pay, &mats_k, &floats_k, fixed_k, face_k)
    });
    for _ in 1..hess_trials {
        hessian = compute_hessian_k::<K, _>(&disc_rates, |v| {
            price_swap(v, is_fixed_pay, &mats_k, &floats_k, fixed_k, face_k)
        });
        black_box(&hessian);
    }
    let hess_elapsed = t_hess.elapsed();
    let hess_avg = hess_elapsed / hess_trials as u32;

    // ========================================================================
    // Output
    // ========================================================================

    println!("Swap Pricer - discount-rate sensitivities (with Gamma)");
    println!("==================================================");
    ...
    println!("Swap value   v (Jet2)      = {:.6}", value_fwd);
    println!();

    println!("Discount rate risk (1bp shift):");
    println!(
        "  {:<4} {:>16} {:>16} {:>16} {:>18}",
        "i", "DV01 (adjoint)", "DV01 (Jet2)", "Gamma (Jet2)", "H[i,i] (JetK)"
    );
    println!(
        "  {:-<4} {:->16} {:->16} {:->16} {:->18}",
        "", "", "", "", ""
    ...
    for i in 0..n_rates {
        ...
        println!(
            ...
            i, dv01_rev, dv01_fwd, gammas[i], hessian[[i, i]]
        );
    }
    ...
    // -------- Consistency checks --------
    let max_delta_err_jet2 = deltas
        .iter()
        .zip(deltas_fwd.iter())
        .fold(0.0_f64, |m, (a, f)| m.max((a - f).abs()));
    // The K-wide engine's diagonal must reproduce the seeded-Jet2 gammas: the
    // two modes compute the same second derivatives by different routes.
    let max_gamma_err = (0..n_rates)
        .map(|i| (hessian[[i, i]] - gammas[i]).abs())
        .fold(0.0_f64, f64::max);
    // The Hessian is symmetric by construction; assert it rather than assume.
    let max_asym = (0..n_rates)
        .flat_map(|i| (0..n_rates).map(move |j| (i, j)))
        .map(|(i, j)| (hessian[[i, j]] - hessian[[j, i]]).abs())
        .fold(0.0_f64, f64::max);

    println!("Max |delta_adj - delta_jet2|      = {:.2e}", max_delta_err_jet2);
    println!("Max |H[i,i] (JetK) - gamma_jet2|  = {:.2e}", max_gamma_err);
    println!("Max |H[i,j] - H[j,i]| (symmetry)  = {:.2e}", max_asym);
    assert!(
        max_delta_err_jet2 < 1e-8,
        "Jet2 delta disagrees with adjoint delta: {}",
        max_delta_err_jet2
    );
    assert!(
        max_gamma_err < 1e-6,
        "K-wide Hessian diagonal disagrees with Jet2 gamma: {}",
        max_gamma_err
    );
    assert!(max_asym == 0.0, "Hessian is not symmetric: {}", max_asym);
    ...
    println!(
        "  {:<46} {:>12.3?}  {:>14.3?}",
        format!("Gamma diagonal only (Jet2, {} fwd passes)", n_rates),
        fwd_gamma_avg,
        fwd_gamma_elapsed
    );
    println!(
        "  {:<46} {:>12.3?}  {:>14.3?}",
        format!("FULL {n_rates}x{n_rates} Hessian (JetK K={K}, {n_blocks} passes)"),
        hess_avg,
        hess_elapsed
    );
    println!(
        "  (the last row is averaged over {} trials, the others over {})",
        hess_trials, N_TRIALS
    );
}
