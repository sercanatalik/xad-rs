...
//!
//!   2. **Forward single-variable (`Jet1`)** — one tape-free forward
//!      pass gives `dP/dy`. This is the idiomatic choice for a
//!      single-input gradient in xad.
//!
//!   3. **Forward second-order (`Jet2`)** — one forward pass gives
...
// ============================================================================
// Forward single-variable pricer (Jet1)
// ============================================================================

fn price_bond_freal(b: Bond, y: &Jet1<f64>) -> Jet1<f64> {
    let c = b.coupon();
    let r = y / b.freq;
    let one_plus = r + 1.0;
    let mut p = Jet1::constant(0.0);
    for t in 1..=b.n_periods {
...
// ============================================================================
// Forward second-order pricer (Jet2)
// ============================================================================
//
// Seed y as a `Jet2::variable`; one forward pass yields value, dP/dy,
// and d²P/dy² in the returned `Jet2`.

fn price_bond_jet2(b: Bond, y: Jet2<f64>) -> Jet2<f64> {
    let c = b.coupon();
    let r = y / b.freq; // Jet2 / f64
    let one_plus = r + 1.0;
    let mut p = Jet2::constant(0.0);
    for t in 1..=b.n_periods {
...
fn main() {
    ...
    // ==================================================================
    // Pass 2 — Forward single-variable dP/dy via Jet1 (tape-free)
    // ==================================================================
    ...
    for _ in 0..N_TRIALS {
        // `Jet1::new(value, 1.0)` seeds y as the active tangent direction.
        let y_fr = Jet1::new(black_box(y_val), 1.0);
    ...
    // ==================================================================
    // Pass 3 — Forward 2nd-order: value + dP/dy + d²P/dy² via Jet2
    // ==================================================================

    let mut price_jet2 = 0.0_f64;
    let mut dpdy_jet2 = 0.0_f64;
    let mut d2pdy2_jet2 = 0.0_f64;

    let t_jet2 = Instant::now();
    for _ in 0..N_TRIALS {
        let y_jet2 = Jet2::variable(black_box(y_val));
        let p = price_bond_jet2(bond, y_jet2);
        price_jet2 = p.value();
        dpdy_jet2 = p.first_derivative();
        d2pdy2_jet2 = p.second_derivative();
        black_box(&d2pdy2_jet2);
    }
    let jet2_elapsed = t_jet2.elapsed();
    let jet2_avg = jet2_elapsed / N_TRIALS as u32;

    // Derived metrics from each mode
    let mod_dur_rev = -dpdy_rev / price_rev;
    let mod_dur_fwd = -dpdy_fwd / price_fwd;
    let mod_dur_jet2 = -dpdy_jet2 / price_jet2;
    let conv_jet2 = d2pdy2_jet2 / price_jet2;
    ...
    println!("Price (Jet1 fwd) = {:.8}", price_fwd);
    println!("Price (Jet2)     = {:.8}", price_jet2);
    println!();

    println!(
        "  {:<22} {:>18} {:>18} {:>18} {:>18}",
        "metric", "analytic", "AReal (rev)", "Jet1 (fwd)", "Jet2 (fwd²)"
    );
    println!(
        "  {:-<22} {:->18} {:->18} {:->18} {:->18}",
        "", "", "", "", ""
    ...
    println!(
        ...
        "dP/dy", dpdy_ref, dpdy_rev, dpdy_fwd, dpdy_jet2
    );
    println!(
        "  {:<22} {:>18.10} {:>18} {:>18} {:>18.10}",
        "d²P/dy²", d2pdy2_ref, "— (1st only)", "— (1st only)", d2pdy2_jet2
    );
    println!(
        "  {:<22} {:>18.10} {:>18.10} {:>18.10} {:>18.10}",
        "modified duration", mod_dur_ref, mod_dur_rev, mod_dur_fwd, mod_dur_jet2
    );
    println!(
        "  {:<22} {:>18.10} {:>18} {:>18} {:>18.10}",
        "convexity", conv_ref, "— (1st only)", "— (1st only)", conv_jet2
    );
    ...
    assert!(jet2_err < tol, "d²P/dy² disagreement: {:.2e}", jet2_err);
    ...
    println!(
        "  {:<46} {:>12.3?}  {:>14.3?}",
        "Forward-mode dP/dy (Jet1, tape-free)", fwd_avg, fwd_elapsed
    );
    println!(
        "  {:<46} {:>12.3?}  {:>14.3?}",
        "Forward 2nd-order dP/dy + d²P/dy² (Jet2)", jet2_avg, jet2_elapsed
    );
}
