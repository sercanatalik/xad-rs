...
//! complementary ways:
//!
//!   1. **Reverse-mode (AReal + tape)** — all 6 inputs (S, K, T, σ, r_d,
//!      r_f) are recorded on a tape and a single reverse sweep yields the
//!      whole 6-D gradient, at a cost independent of the input count.
//!
//!   2. **Forward-mode 2nd order (`Jet2`)** — spot alone is seeded as
...
// ============================================================================
// Forward-mode second order (`Jet2`)
// ============================================================================

/// Prices the option using `Jet2`; by seeding *only spot* as the active
...
fn main() {
    ...
    // ================================================================
    // Pass 1 — Reverse-mode full gradient via `AReal` + fresh tape
    // ================================================================
    ...
    // ================================================================
    // Pass 3 — Spot-gamma via `Jet2` (one forward pass)
    // ================================================================

    let mut price_jet2 = 0.0_f64;
    let mut delta_jet2 = 0.0_f64;
    let mut gamma_jet2 = 0.0_f64;
    ...
    for _ in 0..N_TRIALS {
        ...
        price_jet2 = v.value();
        delta_jet2 = v.first_derivative();
        gamma_jet2 = v.second_derivative();
        black_box(&gamma_jet2);
    }
    ...
    println!("Price (Jet2)     = {:.10}", price_jet2);
    println!();
    ...
    println!(
        "  {:<18} {:>16} {:>16} {:>16}",
        "Greek", "Analytic", "AReal (rev)", "abs. diff"
    );
    println!(
        "  {:-<18} {:->16} {:->16} {:->16}",
        "", "", "", ""
    ...
    for i in 0..6 {
        println!(
            "  {:<18} {:>16.10} {:>16.10} {:>16.2e}",
            names[i], ana[i], grad_rev[i], (grad_rev[i] - ana[i]).abs()
        );
    }
    println!();
    println!("Spot gamma (d²C/dS²):");
    println!("  analytic = {:.10}", gamma_a);
    println!("  Jet2    = {:.10}", gamma_jet2);
    println!("  (Jet2 delta = {:.10}, matches delta column above)", delta_jet2);

    // -------- Cross-check against the analytic reference --------
    let max_err_rev = (0..6)
        .map(|i| (grad_rev[i] - ana[i]).abs())
        .fold(0.0_f64, f64::max);
    let gamma_err = (gamma_jet2 - gamma_a).abs();
    let price_err = (price_rev - price_analytic)
        .abs()
        .max((price_jet2 - price_analytic).abs());
    ...
    assert!(
        gamma_err < tol,
        "Jet2 gamma disagrees with analytic by {:.2e}",
        gamma_err
    );
    ...
    println!(
        "  {:<46} {:>12.3?}  {:>14.3?}",
        "Forward-mode spot gamma (Jet2)", jet2_avg, jet2_elapsed
    );
}
